...
import com.mirbor.blurpreview.AndroidUtils.runOnUIThread
import com.mirbor.blurpreview.App
import com.mirbor.blurpreview.DispatchQueue
import com.mirbor.blurpreview.NativeBlur.blurBitmap
...
class SizeNotifierFrameLayout @JvmOverloads constructor(
    ...
    val backgroundSizeY: Int
        get() {
            ...
            if (backgroundImage is MotionBackgroundDrawable) {
                ...
                offset = if (motionBackgroundDrawable?.hasPattern() == false) {
                    if (animationInProgress) {
                        emojiOffset.toInt()
                    } else if (emojiHeight != 0) {
                        emojiHeight
    ...
    protected val newDrawable: Drawable
        protected get() = ShapeDrawable() //todo get root view
    ...
    fun startBlur() {
       ...
       // if (blurAlpha == 255) {
        ...
        val lastH: Int = AndroidUtils.getStatusBarHeight() + 100.dp
        if (lastW == 0 || lastH == 0) {
            return
        }
                ...
                for (a in 0..1) {
                    ...
                    if (drawable is MotionBackgroundDrawable) {
                        ...
                        if (motionBackgroundDrawable.hasPattern()) {
                            val val1 = (if (isActionBarVisible) AndroidUtils.getStatusBarHeight() else 0)
                            val val2 = if (Build.VERSION.SDK_INT >= 21 && occupyStatusBar) AndroidUtils.getStatusBarHeight() else 0
                            val actionBarHeight: Int = val1 + val2
                        ...
                        if (drawable.tileModeX == Shader.TileMode.REPEAT) {
                            canvas.save()
                            val scale: Float = 2.0f / App.appContext?.resources?.displayMetrics?.density!!
                            ...
                            val val1 = if (isActionBarVisible) AndroidUtils.getStatusBarHeight() else 0
                            val val2 = if (occupyStatusBar) AndroidUtils.getStatusBarHeight() else 0
                            val actionBarHeight: Int =  val1 + val2
                            val viewHeight = rootView.measuredHeight - actionBarHeight
